...
//needs to implement Comparable in order to be used with sets and trees
class Location implements Comparable<Location> {
  ...
  /* compareTo
   *  parameters: 
   *      loc -- Location to be comparedTo
   *  return value: int
   * 
   *  compare method for two Locations
   *  necessary since implementing Comparable
   */
   ...
   *  return value: nothing
   * 
   *  initializes Location object to be ready for iteration
   */
  void start() {  
    // -
    iterationMode = 0; 
   ...
   * 
   *  returns the next neighbor for a given Location's word
   * follows priority of first changing a letter, then adding a later
   * and finally, deleting a letter
   */
  Location nextNeighbor() {  
    //makes temp to change letters for
    Location temp = new Location();
    temp.word = word;
    StringBuilder sb = new StringBuilder(temp.word);
    
    //changes letter of each character in string of Location's word
    if (iterationMode == CHANGE_LETTER) {
      //make sure indexToChange is in range of word
      //set char array to easily replace letter
      //char[] tempWordChange = temp.word.toCharArray();
      
      if(indexToChange < word.length()){
        //make sure word will not equal itself
        if (word.charAt(indexToChange) != nextLetter){
          //tempWordChange[indexToChange] = nextLetter;
          //temp.word = new String(tempWordChange);
          sb.setCharAt(indexToChange, nextLetter);
          temp.word = sb.toString();
          //System.out.println(temp.word);
          //check to see if end of alphabet is reached
          //if so then change indexToChange
          if(alphabet.indexOf(nextLetter+1) >= 0){
            nextLetter++;
          }
          else {
            indexToChange++;
        ...
        //if word is equal to itself then move onto next letter and
      ...
      //when end of word is reached prepare for next insert letter
      else {
    ...
    //inserts a letter into word at given indexToChange
    else if (iterationMode == INSERT_LETTER) {
      //make sure within range
      //StringBuilder sb = new StringBuilder(temp.word);
      if(indexToChange <= word.length()){
        //insert letter
        //temp.word = (temp.word.substring(0,indexToChange) + nextLetter + temp.word.substring(indexToChange, temp.word.length()));
        temp.word = sb.insert(indexToChange, nextLetter).toString();
        //check to see if alphabet is finished
        if(alphabet.indexOf(nextLetter+1) >= 0){
          nextLetter++;
        }
        else {
      ...
      //if end of word reached prepare for delete letter
      else {
        indexToChange = 0;
        nextLetter = 'a';
        iterationMode++;
    ...
    //deletes letter from word
    else if (iterationMode == DELETE_LETTER) {
      //StringBuilder sb = new StringBuilder(temp.word);
      //make sure in range of word
      if(indexToChange < word.length()){
        //delete letter at given indexToChange
        //temp.word = (temp.word.substring(0, indexToChange) + temp.word.substring(indexToChange+1, temp.word.length()));
        temp.word = sb.deleteCharAt(indexToChange).toString();
        indexToChange++;
      }
      //if end is reached prepare to be done
      else{
        indexToChange = 0;
        nextLetter = 'a';
        iterationMode++;
   ...
   * 
   *  checks to see if Location has traversed all neighbors
   */
  boolean isDone() {  
    // -
  ...
  /* isEqual
   *  parameters: 
   *      loc -- Location to compare to
   *  return value: boolean
   * 
   *  checks to see if two Location's words are equal
   */
  ...
  /* streamIn
   *  parameters: 
   *      input -- Scanner holding information
   *  return value: nothing
   * 
   *  takes in input from scanner to set Location's word
   */
   ...
   *  return value: boolean
   *
   *  checks to see if Location is less than another Location
   */
  boolean isLess(Location loc) {
    if(word.compareTo(loc.word) == -1){
      return true;
    }
    else {
      return false;
